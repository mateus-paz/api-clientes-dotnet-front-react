...
function App() {
  ...
  //UseEffect
  useEffect(()=>{
    fetch("https://localhost:7239/api/Clientes")
...
const cadastrar = () => {
  ...
  alert(JSON.stringify(objCadastro));
  fetch('https://localhost:7239/api/Clientes', {
    method:'POST',
...
const consultar = () => {
  ...
  fetch('https://localhost:7239/api/Clientes/'+objCliente.id)
  .then(retorno =>retorno.json())
  .then(retorno_convertido =>{
...
//Pesquisar Por Nome
const pesquisar = () => {
  fetch('https://localhost:7239/api/Clientes/query/'+objCliente.nome)
  .then(retorno =>retorno.json())
  .then(retorno_convertido =>{
